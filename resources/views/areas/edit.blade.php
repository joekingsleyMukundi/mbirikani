            ...
            <div class="row">
                <div class="col-12">
                    <div class="card mt-3">
                        ...
                        <form method="post" action="{{ route('dashboard.areas.update', $area->id) }}">
                            @csrf
                            <div class="card-body">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label for="name">Name<span class="text-danger">*</span></label>
                                        <input autofocus type="text" class="form-control" id="name" name="name" placeholder="Name" value="{{ $area->name }}" required maxlength="255">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" id="asset_use_id" name="asset_use_id">
                                            @foreach($uses as $use)
                                              <option value="{{ $use->id }}" {{ $area->asset_use_id==$use->id ? "selected" : "" }} title="{{ $use->name }}">{{ $use->name }}</option>
                                            @endforeach
                                        </select>
